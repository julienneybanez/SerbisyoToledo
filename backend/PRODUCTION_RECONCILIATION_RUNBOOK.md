...
The active backend now depends on relational tables and columns that were not present in the last committed Railway schema audit, including provider taxonomy/languages/skills, availability, credentials, booking date tables, Messages, contact sharing, and request-linked portfolio data.
...
Do not continue to deployment if the post-audit is incompatible.
...
It also checks that obsolete compatibility columns no longer block modern inserts.
